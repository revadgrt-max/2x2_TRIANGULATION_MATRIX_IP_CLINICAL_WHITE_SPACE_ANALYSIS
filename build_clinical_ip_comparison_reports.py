"""
build_clinical_ip_comparison_reports.py
=========================================
Parses the already-executed notebook (input/clinical_ip_comparison_final_version1.ipynb)
directly -- no recomputation -- so every number, table and chart in the deliverables
mirrors exactly what is in the notebook.

Produces, under output/clinical_ip_comparison_version1/:
    Clinical_IP_Comparison_Report.pdf     - full report: title, clickable TOC, TL;DR,
    ...
    Clinical_IP_Comparison_Report.docx    - same content as the PDF, as an editable Word doc
    Clinical_IP_Comparison_Deck.pptx      - slide deck: title, clickable agenda, TL;DR,
                                            executive summary, one slide per Entry
    Clinical_IP_Comparison_Notebook_Mirror.docx
                                          - full cell-by-cell mirror of the notebook (every
                                            markdown note + every table, up to 50 rows, +
                                            every chart + link to the full CSV)
    clinical_ip_comparison_final_version1.ipynb  - notebook copy (already placed by the
                                            build script; verified/re-copied here)
...
REPORT_TITLE = "Clinical x IP Triangulation -- Antibody Therapeutics in Oncology"
SUBTITLE = "Cross-Notebook Whitespace/Crowded Comparison + Independent Raw-Data Triangulation on target_harmonized"
...
# ============================================================================
# 2. TL;DR + Executive Summary text (pulled verbatim from Entry 18's own
#    printed executive summary)
# ============================================================================
e18 = next((e for e in entries if e["number"] == 18), None)
exec_summary = next((it["text"] for it in (e18["items"] if e18 else []) if it["type"] == "text" and "EXECUTIVE SUMMARY" in it["text"]), "")

TLDR_BULLETS = [
    "8,901 antibody-oncology patent records (ip_final_version3.csv) triangulated against 19,357 antibody-oncology "
    "trial records (clinical_final_version1.csv), joined exclusively on the curated `target_harmonized` column.",
    "Part A reproduces -- does not re-invent -- the whitespace/crowded tier logic already coded in "
    "ip_landscape_final_version1.ipynb (Entry 5D) and clinical_landscape_final_version2.ipynb (Entries 11/13/31), "
    "then cross-compares the two notebooks' own conclusions target-by-target.",
    "9 targets are Crowded in BOTH notebooks (PD-1, PD-L1, HER2, EGFR, CTLA-4, TROP-2, CD20, VEGF, CD19); "
    "25 are Whitespace in both; 66 targets receive a DIFFERENT tier from each notebook.",
    "A dedicated spotlight flags targets the IP notebook calls Crowded where clinical activity has not caught "
    "up, and cross-references the clinical notebook's Phase-1/2-only whitespace targets against IP crowding.",
    "Part B triangulates directly from the raw CSVs: modality-adjusted crowding, filing-to-first-trial timing "
    "lag (flagging targets where trials started before any patent was filed), clinically-validated/patent-thin "
    "targets (strongest 'file now' case), patent-crowded/clinically-unproven targets (highest FTO/litigation "
    "exposure), the CAR-T IP coverage blind spot, and approximate sponsor/assignee overlap.",
    "A solid-tumor-indication dissection (Entry 9, notebook-tier based; Entry 17, raw-data based) breaks down "
    "every headline finding by tumor type using one shared canonical taxonomy across both datasets.",
]
...
story.append(Paragraph(
    f"Report generated: {TODAY}<br/>Source notebook: clinical_ip_comparison_final_version1.ipynb<br/>"
    f"Source data: ip_final_version3.csv (8,901 patents) + clinical_final_version1.csv (19,357 trials)",
    styles["MetaCenter"],
))
...
pdf_path = OUT / "Clinical_IP_Comparison_Report.pdf"
...
docx_path = OUT / "Clinical_IP_Comparison_Report.docx"
doc.save(docx_path)
...
r = mp.add_run("Clinical IP Comparison -- Notebook Mirror")
r.bold = True
r.font.size = Pt(24)
r.font.color.rgb = RGBColor(0x1F, 0x2A, 0x44)
sp = mirror.add_paragraph()
sp.alignment = WD_ALIGN_PARAGRAPH.CENTER
sp.add_run(f"Full cell-by-cell copy of clinical_ip_comparison_final_version1.ipynb -- generated {TODAY}").italic = True
...
for e in entries:
    ...
    if e["code"]:
        mirror.add_heading("Code (SQL + Python)", level=2)
...
mirror_path = OUT / "Clinical_IP_Comparison_Notebook_Mirror.docx"
...
slide = prs.slides.add_slide(BLANK)
box = slide.shapes.add_textbox(PIn(0.8), PIn(2.2), prs.slide_width - PIn(1.6), PIn(1.8))
...
sub_box = slide.shapes.add_textbox(PIn(0.8), PIn(3.9), prs.slide_width - PIn(1.6), PIn(1.4))
...
meta_box = slide.shapes.add_textbox(PIn(0.8), PIn(5.4), prs.slide_width - PIn(1.6), PIn(0.8))
p3 = meta_box.text_frame.paragraphs[0]
p3.text = f"Generated {TODAY}  |  clinical_ip_comparison_final_version1.ipynb"
p3.font.size = PPt(13)
...
pptx_path = OUT / "Clinical_IP_Comparison_Deck.pptx"
prs.save(pptx_path)
...
# ============================================================================
nb_copy_path = OUT / "clinical_ip_comparison_final_version1.ipynb"
